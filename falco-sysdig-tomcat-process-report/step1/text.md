...
1. Create a Falco rule that detects successful process execution from the single-container Pod `tomcat`.
...
- Use a condition equivalent to `evt.type in (execve, execveat) and evt.dir=< and evt.failed=false and container.id != host and container.name=tomcat`.
...
- rule: Tomcat Process Activity
  desc: Detect successful process execution in container tomcat
  condition: evt.type in (execve, execveat) and evt.dir=< and evt.failed=false and container.id != host and container.name=tomcat
