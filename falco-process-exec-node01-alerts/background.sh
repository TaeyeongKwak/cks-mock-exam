...
mkdir -p /opt/node-01/alerts /etc/falco /etc/falco/config.d /var/log/falco
rm -f /opt/node-01/alerts/details /var/log/falco/falco-json.log
...
cat >/usr/local/bin/falco-report-csv <<'EOF_HELPER'
#!/bin/bash
set -euo pipefail

if [ "$#" -ne 2 ]; then
  echo "usage: falco-report-csv <duration-seconds> <output-file>" >&2
  exit 1
fi
...
with open(path, 'r', encoding='utf-8', errors='ignore') as fh:
    for line in fh:
        ...
        if ts and uid is not None and proc:
            print(f"{ts},{uid},{proc}")
PY
EOF_HELPER

chmod 0755 /usr/local/bin/falco-report-csv
systemctl daemon-reload
systemctl enable falco.service >/dev/null 2>&1 || true
systemctl restart falco.service >/dev/null 2>&1 || true
EOF

kubectl delete namespace runtime-lab --ignore-not-found >/dev/null 2>&1 || true
kubectl create namespace runtime-lab >/dev/null
...
kubectl wait --for=condition=Available deployment/exec-spawner -n runtime-lab --timeout=180s >/dev/null
